...
The code constructed the data path as f"{data}_data.pt", producing filenames like cells_data.pt. The actual files on disk were named cells.pt, chest.pt, lesions.pt on _data suffix. Every run crashed with FileNotFoundError before the pipeline could do anything at all.
